...
char const*const CAPS_FILE = "/sys/class/leds/shift-key-light/brightness";
        ...
        struct light_state_t const* state)
{
	...
	if (g_caps_on < 0 ||
	    (g_caps_on == 0 && brightness > 0) ||
	    (g_caps_on > 0 && brightness == 0))
	{
		err = write_int(CAPS_FILE, brightness ? 255 : 0);
	}

	g_caps_on = brightness != 0;
...
struct hw_module_t HAL_MODULE_INFO_SYM = {
	...
	.name = "Motorola msm8960 lights Module",
